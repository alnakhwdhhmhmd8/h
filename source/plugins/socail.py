...
async def msong(app, msg):
    ...
    if "https://www.instagram.com/" in link :
        ...
        if rr["status"] == "Done":
            ...
            copy=f"""↯ | Dev⇾ [𝐆 𝐀 𝐊 - مــانــدو](t.me/wwvvwl)"""
            try:
...
def insts(link):
    ...
    if res["status"] == "ok":
        ...
        return {"status":"Done","vedio":vedio,"POWEREDBY":"@wwvvwl"}
    else:
        return {"status":"none","POWEREDBY":"@wwvvwl"}
...
def social(url):
	try:
		...
		if not dl.get("message"):
			return {"status":"Done", "vedio": dl["result"]["url"], "POWEREDBY":"@wwvvwl"}
		else:
			return {"status":"none", "POWEREDBY":"@wwvvwl"}
