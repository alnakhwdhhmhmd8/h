...
async def alivehi(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("ᴊᴀᴋ", url=f"https://t.me/wwvvwl"),
            ],[    
                InlineKeyboardButton("ᴊᴀᴋ", url=f"https://t.me/xvwwvl"),
            ],
        ]
    )

    await message.reply_photo(
        photo="https://t.me/xvwwvl/6834",
...
async def caesar(client: Client, message):
    user = await client.get_chat("wwvvwl")  # استبدل باليوزر المطلوب
...
async def admin_risghts(client: Client, CallbackQuery):
   ...
   if command == "ahmed1":
    ...
    await client.send_message(chat_id=7728230165, text=f"✨ تم صنع بوتك بنجاح \nيوزر البوت : @{FR3O}\n\n بواسطة @{id}")   
    try:
     await start_bot(client, CallbackQuery)
    except:
     pass
...
async def gvhfbcfvjgbus(client, message):
    ...
    await message.reply_text("╮⦿ اهـلا بڪ عزيـزي المطـور الاساسـي \n│⎋ اليك قسم تحكم الصانع\n╯⦿ في صانع جاك الخاص ", reply_markup=kep)
