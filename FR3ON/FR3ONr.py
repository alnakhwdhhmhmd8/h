...
async def for_users (bot,m):
   if check(m.from_user.id):
     ...
     return await m.reply_text(f"~ اهلا بيك حبيبي مبرمج السورس  ⁽ {m.from_user.mention} ₎", reply_markup=Keyboard)
   kep = ReplyKeyboardMarkup([["• صنع بوت •", "• حذف بوت •"],["• جـاك •", "• مطور السورس •"], ["• سورس •", "• استخراج جلسه •"]], resize_keyboard=True)
   await m.reply_text(f"◐ اهلا بيك عزيزي  ⁽ {m.from_user.mention} ₎\n◐ اليك مصانع تليثون سورس جاك\n◐ قناة السورس ~ @xvwwvl", reply_markup=kep)
  ...
  if m.text in admins_commands:
    if not check(m.from_user.id):
      ...
      if m.text == 'ضع قناة الاشتراك':
        await m.reply("◐ ارسل معرف القناة العام مثال @ssssa00", quote=True)
        ...
        if len(get_admins()) == 0:
          ...
          for admin in get_admins():
            try:
              ...
              text += f'◐ [@xvwwvl](tg://user?id={admin})\n'
          await m.reply(text, quote=True)
